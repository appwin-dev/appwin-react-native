...
internal class AppwinSupportModule(
  ...
  fun initialize(promise: Promise) =
    resolving(promise, "availability_failed") { encodeInitResult(AppwinSupport.initialize()) }

  @ReactMethod
  fun isReady(promise: Promise) {
    promise.resolve(AppwinSupport.isReady)
  }

  @ReactMethod
  fun version(promise: Promise) {
    promise.resolve(AppwinSupport.VERSION)
  }
  ...
  @ReactMethod
  fun presentConversation(conversationId: String, promise: Promise) {
    val activity = reactApplicationContext.currentActivity
    if (activity == null) {
      promise.reject("appwin_no_activity", "Aucune activité au premier plan")
      return
    }
    AppwinSupport.presentConversation(activity, conversationId)
    promise.resolve(null)
  }

  @ReactMethod
  fun loginUnidentifiedUser(promise: Promise) =
    resolving(promise, "appwin_login_failed") {
      val customer = AppwinSupport.loginUnidentifiedUser()
    ...
    resolving(promise, "appwin_update_failed") {
      val customer = AppwinSupport.updateUser(
        SupportUserAttributes(
          ...
          plan = attributes.getString("plan"),
        ),
      )
      Arguments.createMap().apply {
        putString("id", customer.id)
...
internal class AppwinNotificationsModule(
  ...
  @ReactMethod
  fun initialize(promise: Promise) =
    resolving(promise, "availability_failed") { encodeInitResult(AppwinNotifications.initialize()) }

  @ReactMethod
  fun start(requestPushPermission: Boolean, promise: Promise) =
    resolving(promise, "start_failed") {
      AppwinNotifications.start(reactApplicationContext)
    }

  @ReactMethod
  fun stop(promise: Promise) =
    resolving(promise, "stop_failed") {
  ...
  @ReactMethod
  fun trackEvent(event: String, eventName: String?, properties: ReadableMap?, promise: Promise) {
    ...
    val props = properties?.toHashMap()?.mapNotNull { (key, value) ->
      (value as? String)?.let { key to it }
    }?.toMap()
    resolving(promise, "appwin_track_event_failed") {
      AppwinNotifications.trackEvent(automationEvent, eventName, props)
    }
  }

  @ReactMethod
  fun track(deliveryId: String, event: String, buttonIndex: Double?, promise: Promise) {
    val trackEvent = TrackEvent.entries.firstOrNull { it.wireValue == event }
    if (trackEvent == null) {
      promise.reject("appwin_unknown_event", "Événement inconnu : $event")
      return
    }
    val index = buttonIndex?.toInt()
    resolving(promise, "appwin_track_failed") {
      AppwinNotifications.track(deliveryId, trackEvent, index)
    }
  }
  ...
  @ReactMethod
  fun presentPendingMessages(promise: Promise) =
    resolving(promise, "appwin_present_failed") {
              ...
              buttons.forEach { button ->
                pushMap(
                  Arguments.createMap().apply {
                    ...
                    putString("action", button.action)
                    putString("url", button.url)
                  },
                )
              }
            },
 ...
 * two must not drift apart across the bridge.
 */
private fun encodeInitResult(result: AppwinInitResult): WritableMap =
  Arguments.createMap().apply {
    when (result.status) {
      AppwinInitStatus.READY -> putString("status", "ready")
      AppwinInitStatus.NOT_CONFIGURED -> putString("status", "notConfigured")
      AppwinInitStatus.UNKNOWN -> putString("status", "unknown")
      AppwinInitStatus.UNAVAILABLE -> {
        putString("status", "unavailable")
        putString("reason", result.reason?.key ?: "disabled")
      }
    }
  }


internal class AppwinAnalyticsModule(
  context: ReactApplicationContext,
) : AppwinBaseModule(context) {

  override fun getName(): String = "AppwinAnalyticsModule"

  @ReactMethod
  fun initialize(promise: Promise) =
    resolving(promise, "availability_failed") { encodeInitResult(AppwinAnalytics.initialize()) }

  @ReactMethod
  fun isReady(promise: Promise) {
    promise.resolve(AppwinAnalytics.isReady)
  }

  @ReactMethod
  fun track(name: String, props: ReadableMap?, promise: Promise) {
    AppwinAnalytics.track(name, props?.toHashMap())
    promise.resolve(null)
  }

  @ReactMethod
  fun screen(name: String, promise: Promise) {
  ...
  @ReactMethod
  fun setConsent(consent: String, promise: Promise) {
    AppwinAnalytics.setConsent(
      when (consent) {
...
internal class AppwinAttributionModule(
  context: ReactApplicationContext,
) : AppwinBaseModule(context) {

  override fun getName(): String = "AppwinAttributionModule"

  @ReactMethod
  fun initialize(promise: Promise) =
    resolving(promise, "availability_failed") { encodeInitResult(AppwinAttribution.initialize()) }

  @ReactMethod
  fun isReady(promise: Promise) {
    promise.resolve(AppwinAttribution.isReady)
  }

  @ReactMethod
  fun setAdvertisingConsent(consent: String, promise: Promise) {
  ...
  // The JS layer short-circuits this on Android (no ATT); kept for a caller
  // that reaches the native module directly.
  @ReactMethod
  ...
  @ReactMethod
  fun setAdSignalsDebugMode(enabled: Boolean, promise: Promise) {
