...
export const AppwinCore = {
  ...
  bootstrapSession(): Promise<string> {
    // An empty string means the session could not open (offline, broken
    // native install): the no-throw contract forbids rejecting here.
    return guarded('bootstrapSession', () => invoke('core', 'bootstrapSession'), '')
  },
  ...
  identify(externalId: string): Promise<void> {
    return guarded('identify', () => invoke('core', 'identify', externalId), undefined)
  },

  /** Goes back to anonymous locally, without revoking the server session. */
  clearIdentity(): Promise<void> {
    return guarded('clearIdentity', () => invoke('core', 'clearIdentity'), undefined)
  },
  ...
  signOut(): Promise<void> {
    return guarded('signOut', () => invoke('core', 'signOut'), undefined)
  },
