...
export const NATIVE_MODULE_NAMES = {
  ...
  analytics: 'AppwinAnalyticsModule',
  attribution: 'AppwinAttributionModule',
} as const
...
/**
 * Facade no-throw contract: a bridge failure (missing native module, rejected
 * promise) degrades to `fallback` instead of reaching the host app.
 * Integrators never need to wrap Appwin calls in try/catch. Internal: not
 * re-exported by the package entry point.
 */
export async function guarded<T>(
  ...
  try {
    ...
    // `typeof` keeps the catch path safe in Node test runs, where the React
    // Native `__DEV__` global does not exist.
    if (typeof __DEV__ !== 'undefined' && __DEV__) {
      console.warn(`[Appwin] ${method}: bridge call failed:`, e)
    }
    return fallback
  }
}
